...
@export var simulation_chunk_size_tiles: int = 32
...
func _ready() -> void:
	...
	_last_processed_minute = get_current_total_minutes()

# Converts a 3D world coordinate to a 2D grid coordinate (1 meter = 1 tile)
...
func get_current_total_minutes() -> int:
	return ((TimeManager.current_day - 1) * 24 * 60) + (TimeManager.current_hour * 60) + TimeManager.current_minute
...
func set_tile_state(grid_pos: Vector2i, new_state: int, world_height: float = NAN) -> void:
	...
	emit_signal("tile_updated", grid_pos, new_state)
