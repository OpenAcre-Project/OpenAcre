...
func _resolve_wheel_reference(existing_wheel: Wheel, fallback_node_name: String) -> Wheel:
	if existing_wheel != null:
		return existing_wheel

	var fallback_node := get_node_or_null(fallback_node_name)
	if fallback_node is Wheel:
		return fallback_node

	return null

func initialize():
	front_left_wheel = _resolve_wheel_reference(front_left_wheel, "WheelFrontLeft")
	front_right_wheel = _resolve_wheel_reference(front_right_wheel, "WheelFrontRight")
	rear_left_wheel = _resolve_wheel_reference(rear_left_wheel, "WheelRearLeft")
	rear_right_wheel = _resolve_wheel_reference(rear_right_wheel, "WheelRearRight")

	if front_left_wheel == null or front_right_wheel == null or rear_left_wheel == null or rear_right_wheel == null:
		push_error("Vehicle is missing one or more wheel references. Assign front/rear left/right wheels in the inspector.")
		return
