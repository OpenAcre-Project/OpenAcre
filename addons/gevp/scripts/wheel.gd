...
var vehicle : Vehicle
var _visual_base_quaternion := Quaternion.IDENTITY
var _visual_spin_angle := 0.0

func _ready() -> void:
	# Ensure the suspension catches terrain hits even if the origin briefly enters the mesh at high speeds
	hit_from_inside = true
	
	# Fallback for scenes where wheel_node export was not resolved yet.
	if wheel_node == null and has_node("MeshInstance3D"):
		var fallback_node := get_node("MeshInstance3D")
		if fallback_node is Node3D:
			wheel_node = fallback_node

func _process(delta : float) -> void:
	if wheel_node:
		wheel_node.position.y = minf(0.0, -spring_current_length)
		# Compose orientation from stable channels to avoid Euler precession wobble.
		_visual_spin_angle = wrapf(_visual_spin_angle - (spin * delta), -PI, PI)

		var steering_angle := rotation.y * visual_steering_multiplier
		var steering_quat := Quaternion(_axis_from_enum(visual_steering_axis), steering_angle)
		var spin_quat := Quaternion(_axis_from_enum(visual_spin_axis), _visual_spin_angle)

		var beam_roll_angle := 0.0
		if not is_zero_approx(beam_axle) and opposite_wheel != null and opposite_wheel.wheel_node != null:
			var wheel_lookat_vector := (opposite_wheel.transform * opposite_wheel.wheel_node.position) - (transform * wheel_node.position)
			beam_roll_angle = wheel_lookat_vector.angle_to(Vector3.RIGHT * beam_axle) * signf(wheel_lookat_vector.y * beam_axle)
		var beam_roll_quat := Quaternion(Vector3.FORWARD, beam_roll_angle)

		var composed := _visual_base_quaternion * steering_quat * beam_roll_quat * spin_quat
		wheel_node.basis = Basis(composed)
...
func initialize() -> void:
	if wheel_node == null:
		if has_node("MeshInstance3D"):
			...
			if fallback_node is Node3D:
				wheel_node = fallback_node
	if wheel_node == null:
		push_warning("Wheel '%s' is missing wheel_node; visual wheel updates disabled." % name)
		return

	wheel_node.rotation_order = EULER_ORDER_ZXY
	_visual_base_quaternion = wheel_node.quaternion
	_visual_spin_angle = 0.0
	...
	if not _is_valid_surface(surface_type):
		if not tire_stiffnesses.is_empty():
			surface_type = String(tire_stiffnesses.keys()[0])
	_apply_surface_parameters(surface_type)
...
func process_forces(opposite_compression : float, braking : bool, delta : float) -> float:
	...
	if is_colliding():
		...
		if surface_groups.size() > 0:
			var detected_surface := _resolve_surface_type(surface_groups)
			if not detected_surface.is_empty() and surface_type != detected_surface:
				surface_type = detected_surface
				_apply_surface_parameters(surface_type)
	else:
		last_collider = null
...
func _axis_from_enum(axis: int) -> Vector3:
	match axis:
		0:
...
func _resolve_surface_type(surface_groups: Array[StringName]) -> String:
	for group_name: StringName in surface_groups:
		var candidate := String(group_name)
		if _is_valid_surface(candidate):
			return candidate
	return ""

func _is_valid_surface(surface_name: String) -> bool:
	return tire_stiffnesses.has(surface_name) \
		and coefficient_of_friction.has(surface_name) \
		and rolling_resistance.has(surface_name) \
		and lateral_grip_assist.has(surface_name) \
		and longitudinal_grip_ratio.has(surface_name)

func _apply_surface_parameters(surface_name: String) -> void:
	if not _is_valid_surface(surface_name):
		return

	current_cof = float(coefficient_of_friction.get(surface_name, 1.0))
	current_rolling_resistance = float(rolling_resistance.get(surface_name, 1.0))
	current_lateral_grip_assist = float(lateral_grip_assist.get(surface_name, 0.0))
	current_longitudinal_grip_ratio = float(longitudinal_grip_ratio.get(surface_name, 0.5))
	current_tire_stiffness = 1000000.0 + 8000000.0 * float(tire_stiffnesses.get(surface_name, 1.0))
